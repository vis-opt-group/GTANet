...
def calc_psnr(img1, img2):
    ...
    :param img1: im_true, 图片
    :param img2: im_test, 图片
    :return:
    '''
    # 只需要计算明度即可
    y1 = np.array(img1.convert('YCbCr'))[:, :, 0]
    y2 = np.array(img2.convert('YCbCr'))[:, :, 0]
    # compare_psnr(im_true, im_test, data_range=None)
...
def fast_extract_video(video_name, save_dir='', extract_music=False, music_name=''):
    ...
    flag = subprocess.call(command,
                           shell=True)  # ffmpeg -i F:\TOFlow\test.mp4 -f image2 F:\TOFlow\frame\test\%06d.png -s 1920*1080 -r 24
    if flag:
